...
def session_start_route():
	...
	# user default data
	# session['password'] = None idk if use it
	...
	session['language'] = language_form  # directly declare language on server theread.
...
# WORKING
# basically it returns a server_id and from it you know which server delete from a array
...
# WORKING
# i return game version and if user change i get version = new version, server_id = destination server
...
def servers_versions_route():
	...
	# if user touch change version
	#global servers
	# for server in servers:
	#	if server['id'] == request.form['server_id']:
	#		if server['version'] != request.form['version']:
...
# WORKING with some useless posts from app.. noob devs
# it return ip = string, port = string, server_id = string server name
...
# it returns id = server id maybe?, language = lang to set, server_id = server name or id
# USELESS SERVER ID DUPLICATE
...
# WORKING
# it changes server name, it returns 2 keys: name = new server name & server_id = destination server where change
	...
	for server in session['servers']:
		if server['id'] == request.form['server_id']:
			if server['serverName'] != request.form['name']:
				...
				return '', 204  # to figure out for each function
			else:
				return  # idk how to return error if same name
		else:
			return  # idk how to return error if server bugs lmao


# TODO finish logic, op player, make ip, server id
# it returns server_name = string with name, game_mode = int gamemode, server_owner = string owner player
...
# WORKING
# EACH DOMAIN IN DOMAINS GLOBAL VARIABLE HAVE TO BE IN FORMAT ARRAY WITH INSIDE STRING "domain" EQUALS DOMAIN NAME
...
def user_gift_route():
	...
	for key in request.form:
		...
		# update userdatat set credits = json.ddd(dadas array con int) where server ip = ip
	return '', 204
...
def earning_social_youtube_route():
	...
	for key in request.form:
		print(f'{key}: {request.form[key]}')
	# i have to undertsnad which reply send to set already done
	response = [[False]]
...
def earning_social_facebook_route(platform):
	...
	# i have to understand which reply send to set already done
	response = [[None]]
	return json.dumps(response)
...
# WORKS .. DO REAL DOMAIN SEND THROUGH HTTP
# it return ip, port, domain = new domain, server_id = server id
